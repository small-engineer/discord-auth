...
const log_1 = require("../utils/log"); // ログ送信関数を利用
// ユーザーごとのメンション数管理用
...
// 警告処理
async function applyWarning(client, message, member, reason) {
    try {
        // 認証ロールを剥奪
        ...
        if (message.channel.isTextBased() && !message.channel.isDMBased()) {
            await message.channel.send(`<@${member.id}> さん、${reason} のため警告ロールを付与しました。`);
        }
        // ログをチャンネルに送信
        const logMessage = `[WARNING] ${member.user.tag} に警告ロールを付与: ${reason}`;
        console.log(logMessage);
        (0, log_1.logToChannel)(client, logMessage);
    }
    catch (error) {
